...
export const handle = (socket: Socket): void => {
    ...
    socket.on(TopLevelClientToServerMessages.CREATE_GAME, handleError(socket, (...args: any[]) => {
        createGame(socket, ...args);
    }));
...
export const handleError = (socket: Socket, handler: SocketHandler): SocketHandler => {
    return async (...args: any[]) => {
        try {
            ...
            logger.error('Unhandled error in socket handler', socket.id, error);
